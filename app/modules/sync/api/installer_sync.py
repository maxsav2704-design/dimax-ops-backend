...
from app.modules.sync.api.installer_schemas import (
    InstallerSyncBatchRequest,
    InstallerSyncBatchResponse,
    InstallerSyncBatchResultDTO,
...
@router.post("/sync-queue/batch", response_model=InstallerSyncBatchResponse)
def sync_queue_batch(
    body: InstallerSyncBatchRequest,
    user: CurrentUser = Depends(require_installer),
    installer_id: UUID = Depends(get_current_installer_id),
    uow=Depends(get_uow),
):
    with uow:
        res = InstallerSyncService.sync_batch(
            uow,
            company_id=user.company_id,
            installer_id=installer_id,
            actor_user_id=user.id,
        ...
        return InstallerSyncBatchResponse(
            server_time=res["server_time"],
            results=[InstallerSyncBatchResultDTO(**x) for x in res["results"]],
        )
